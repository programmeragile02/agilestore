...
//   const hit = detail?.durations?.find((d) => Number(d.length) === Number(months));
//   return hit?.code || ""; // "M1" | "M6" | "M12"
// }
// function buildPriceMap(detail: ProductDetail) {
//   const sortedDurations = [...(detail.durations || [])].sort((a, b) => a.length - b.length);
...
//                 readOnly
//                 className={`pl-10 ${data.email && !validateEmail(data.email) ? "border-red-300" : ""}`}
//                 required
//               />
//             </div>
...
//                 className={`pl-10 ${data.phone && !validatePhone(data.phone) ? "border-red-300" : ""}`}
//                 required
//               />
//             </div>
...
// }) {
//   const set = (patch: Partial<CheckoutData["plan"]>) => onChange({ ...data, ...patch });

//   const packages = productDetail?.packages ?? [];
//   const durations = useMemo(
//     () => (productDetail?.durations ?? []).slice().sort((a, b) => a.length - b.length),
//     [productDetail]
//   );
...
//           {!data.product ? (
//             <Select value={data.product} onValueChange={(value) => onChangeProduct(value)}>
//               <SelectTrigger className="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 h-10">
//                 <SelectValue placeholder={loading.products ? "Loading products..." : "Select a product"} />
//               </SelectTrigger>
//               <SelectContent className="min-w-[400px]">
//                 {products.map((p) => (
//                   <SelectItem key={p.product_code} value={p.product_code} className="py-4 px-4">
//                     <div className="py-2 px-1">
//                       <div className="font-medium text-base">{p.product_name}</div>
//                       {!!p.description && (
//                         <div className="text-sm text-gray-500 mt-2 leading-relaxed">{p.description}</div>
//                       )}
//                     </div>
//                   </SelectItem>
//                 ))}
...
//               <div className="font-bold text-lg text-gray-900">{selectedProductName}</div>
//               <div className="text-sm text-gray-600 mt-1">{productDetail?.product?.description}</div>
//             </div>
//           )}
//         </div>
...
//               <SelectValue
//                 placeholder={
//                   !data.product ? "Pilih produk terlebih dahulu" : loading.detail ? "Loading packages..." : "Select a package"
//                 }
//               />
//             </SelectTrigger>
//             <SelectContent className="min-w-[350px]">
//               {packages.map((pkg) => (
//                 <SelectItem key={pkg.package_code} value={pkg.package_code} className="py-4 px-4">
...
//                     </div>
//                     {pkg.status === "active" && <Badge className="bg-indigo-100 text-indigo-800 ml-2">Active</Badge>}
//                   </div>
//                 </SelectItem>
//               ))}
//             </SelectContent>
...
//               {packages.find((p) => p.package_code === data.package)?.description}
//             </p>
//           )}
//         </div>
...
//               const disabled = !data.package || !(priceMap[data.package]?.[d.length] > 0);
//               return (
//                 <Button
//                   key={d.code}
...
//                     active ? "bg-gradient-to-r from-indigo-500 to-blue-500 text-white" : "border-gray-300 text-gray-700 hover:bg-gray-50"
//                   }`}
//                 >
...
//                       Rp {priceMap[data.package][d.length].toLocaleString("id-ID")}
//                     </span>
//                   )}
//                 </Button>
//               );
...
//             <Select value={data.currency} onValueChange={(value: "IDR" | "USD") => set({ currency: value })}>
...
//             <Select value={data.taxMode} onValueChange={(value: "inclusive" | "exclusive") => set({ taxMode: value })}>
//               <SelectTrigger className="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 h-10">
...
//               </div>
//             </div>
//             <Button variant="ghost" size="sm" onClick={handleRemoveVoucher} className="text-green-700 hover:text-green-900 hover:bg-green-100">
//               Remove
//             </Button>
//           </div>
//         )}
...
//     plan.currency === "IDR" ? `IDR ${price.toLocaleString("id-ID")}` : `$${(price / 15000).toFixed(2)}`;

//   const getDurationLabel = (m: number) =>
//     m === 1 ? "1 Month" : m === 6 ? "6 Months" : m === 12 ? "12 Months" : `${m} Months`;

//   const getPackageLabel = (pkg: string) =>
//     productDetail?.packages.find((p) => p.package_code === pkg)?.name ?? pkg.charAt(0).toUpperCase() + pkg.slice(1);
...
//           <div className="space-y-3">
//             <div className="mb-3">
//               <h3 className="font-bold text-base text-gray-900">{productName}</h3>
//               {!!productDetail?.product?.description && (
//                 <p className="text-sm text-gray-600 mt-1">{productDetail.product.description}</p>
//               )}
//             </div>
//             <div className="flex items-center gap-2">
//               {!!plan.package && (
...
//             <span className="font-medium text-gray-600">{formatPrice(basePrice)}</span>
//           </div>
...
// // ============================================================================
// // MAIN CLIENT COMPONENT (tanpa Header/Footer!)
// // ============================================================================
...
//   const [priceMap, setPriceMap] = useState<Record<string, Record<number, number>>>({});
...
//           const isEmpty =
//             !prev.contact.fullName && !prev.contact.email && !prev.contact.phone && !prev.contact.company;
//           return isEmpty ? { ...prev, contact: { ...prev.contact, ...contactFromProfile } } : prev;
//         });
//       } catch {}
//     })();
...
//   // load product detail when product changes
//   useEffect(() => {
//     let mounted = true;
...
//       setPriceMap({});
//       setCheckoutData((prev) => ({ ...prev, plan: { ...prev.plan, package: "", duration: 0 } }));
//       return;
//     }
//     (async () => {
...
//         const pkgOk = detail.packages.some((x) => x.package_code === checkoutData.plan.package);
//         const durOk = detail.durations.some((d) => d.length === checkoutData.plan.duration);
...
//             duration: durOk ? prev.plan.duration : detail.durations[0]?.length ?? 0,
//           },
//         }));
//       } catch {
//         setProductDetail(null);
...
//   const onChangePlan = (plan: CheckoutData["plan"]) => setCheckoutData((prev) => ({ ...prev, plan }));

//   const handlePlaceOrder = async () => {
//     setIsPlacing(true);
//     try {
//       const { product, package: pkg, duration } = checkoutData.plan;
//       if (!product || !pkg || !duration) throw new Error("Pilih produk, paket, dan durasi dahulu.");
...
//                   setCheckoutData((prev) => ({
//                     ...prev,
//                     plan: { ...prev.plan, product: productCode, package: "", duration: 0 },
//                   }));
//                 }}
//                 onChangePackage={(packageCode) => {
//                   const firstDur =
//                     productDetail?.durations?.slice().sort((a, b) => a.length - b.length)[0]?.length ?? 0;
//                   setCheckoutData((prev) => ({
//                     ...prev,
//                     plan: { ...prev.plan, package: packageCode, duration: firstDur },
//                   }));
//                 }}
//               />
