...
/** ==== Types & fallback ==== */
type ContactSocials = { instagram?: string; tiktok?: string; youtube?: string };
type ContactContent = {
  email?: string;
  phone?: string;
  address?: string;
...
const FALLBACK: Required<Omit<ContactContent, "socials">> & {
  socials: Required<ContactSocials>;
} = {
  email: "admin@agilestore.com",
  phone: "085702212770",
  address: "Boyolali, Jawa Tengah",
...
const UI = {
  en: {
    ...
    followUs: "Follow us",
    mapTip: "Tip: Drag the map or zoom in/out to explore the surrounding area.",
    showingMap: "Showing map for:",
  },
  id: {
    email: "Email",
    phone: "Telepon",
    ...
    followUs: "Ikuti kami",
    mapTip: "Tip: Geser peta atau perbesar/perkecil untuk menjelajah sekitar.",
    showingMap: "Menampilkan peta untuk:",
  },
} as const;

function readLangFromCookies(): "en" | "id" {
  const store = cookies();
  const raw =
...
/** ==== Ambil content sesuai bahasa ==== */
async function getContact(lang: "en" | "id") {
  // Ambil section "contact" dari API
  const sec: any = await AgileStoreAPI.getSection<ContactContent>("contact");

  // Anggap backend bisa punya variasi:
  // - sec.content (ID) + sec.content_en (EN), atau
  // - sec.content.en & sec.content.id, atau hanya salah satunya.
  const baseEN =
  ...
  const baseID =
    ...
    sec?.content?.id ??
    sec?.content?.["id"] ??
    // jika tidak ada varian id eksplisit, asumsikan `sec.content` itu ID
    sec?.content ??
    null;

  const chosen =
    lang === "en" ? baseEN ?? baseID ?? {} : baseID ?? baseEN ?? {};
  const socialsSrc =
    chosen?.socials ?? sec?.socials ?? baseEN?.socials ?? baseID?.socials ?? {};

  const data: ContactContent = {
    email: chosen?.email ?? FALLBACK.email,
    phone: chosen?.phone ?? FALLBACK.phone,
    address: chosen?.address ?? FALLBACK.address,
    ctaLabel:
      chosen?.ctaLabel ?? (lang === "en" ? FALLBACK.ctaLabel : "Kirim Pesan"),
    headline:
      chosen?.headline ?? (lang === "en" ? FALLBACK.headline : "Hubungi kami"),
    subheadline:
      chosen?.subheadline ??
      (lang === "en"
        ? FALLBACK.subheadline
        : "Kami senang mendengar dari Anda. Hubungi tim kami kapan saja."),
    socials: {
      instagram: socialsSrc.instagram ?? FALLBACK.socials.instagram,
      tiktok: socialsSrc.tiktok ?? FALLBACK.socials.tiktok,
      youtube: socialsSrc.youtube ?? FALLBACK.socials.youtube,
    },
  };

  return data as Required<ContactContent>;
}
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <Link
                      ...
                      className="inline-flex items-center text-sm mt-2 text-emerald-600 hover:underline"
                    >
                      {t.callNow} <ArrowRight className="ml-1 h-3.5 w-3.5" />
                    </Link>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <Link
                      ...
                      className="inline-flex items-center text-sm mt-2 text-violet-600 hover:underline"
                    >
                      {t.openInMaps} <ArrowRight className="ml-1 h-3.5 w-3.5" />
                    </Link>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border bg-white p-6 shadow-sm">
                <p className="font-medium text-foreground mb-3">{t.followUs}</p>
            ...
            <div className="lg:col-span-2">
              ...
              <p className="text-xs text-muted-foreground mt-3">{t.mapTip}</p>
            </div>
          </div>
        </div>
      </section>
